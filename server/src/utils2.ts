...
export function sendFile(path: string, file: string, response: Response) {
    ...
    response.sendFile(
        ...
        function(err: Error | any) {
            if (err && response.statusCode !== constants2.HttpStatus.NOT_MODIFIED && err.code !== "ECONNABORT") {
                if (response.statusCode === constants2.HttpStatus.NOT_FOUND && file !== placeholder) {
                    ...
                    if(err.status !== undefined) {
                        ...
                        response.status(constants2.HttpStatus.NO_CONTENT).send("");
                    }
                }
            }
        }
